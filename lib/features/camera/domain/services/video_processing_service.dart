...
class OverlayContent {
  ...
  bool get isEmpty =>
      strokes.isEmpty && stickers.isEmpty && textOverlays.isEmpty;
...
class VideoProcessingService {
  ...
  static Future<File?> extractAudio(String videoPath) async {
    try {
      ...
      debugPrint(
        'FFmpeg: Fast audio copy failed, trying AAC encode fallback...',
      );
  ...
  static Size calculateTargetResolution(
    Size sourceSize, {
    ...
    final double targetWidth = sourceSize.width > maxWidth
        ? maxWidth
        : sourceSize.width;
  ...
  static Future<File?> createOverlayImage({
    ...
    try {
      ...
      double mapX(double normalizedX) =>
          normalizedX * (renderSize.width * scale) + offsetX;
      double mapY(double normalizedY) =>
          normalizedY * (renderSize.height * scale) + offsetY;

      // Base width for scaling (matches DrawingPainter strategy)
      final videoScaleFactor = videoSize.width / 360;

      debugPrint(
        'Overlay: renderSize=$renderSize, videoSize=$videoSize, scale=$scale, offsetX=$offsetX, offsetY=$offsetY',
      );
      ...
      for (final stroke in content.strokes) {
        ...
        if (stroke.points.length > 1) {
          final path = Path();
          path.moveTo(
            mapX(stroke.points.first.dx),
            mapY(stroke.points.first.dy),
          );
        ...
        final textPainter = TextPainter(
          text: TextSpan(
            ...
            style: AppTypography.displayMedium.copyWith(
              fontSize: 48 * sticker.scale * videoScaleFactor,
            ),
          ),
      ...
      for (final textItem in content.textOverlays) {
        ...
        final style = textItem.fontStyle
            .getStyle(textItem.fontSize * videoScaleFactor, textItem.color)
            .copyWith(
              shadows: [
                Shadow(
        ...
        textPainter.paint(
          canvas,
          Offset(mapX(textItem.position.dx), mapY(textItem.position.dy)),
        );
      }
      ...
      final tempDir = await getTemporaryDirectory();
      final fileName =
          'vibe_overlay_${DateTime.now().millisecondsSinceEpoch}.png';
  ...
  static Future<File?> compositeVideoWithOverlay({
    ...
    try {
      ...
      while (retryCount < maxRetries) {
        ...
        if (exists && size > 0) {
          debugPrint(
            'FFmpeg: Overlay PNG validated ($size bytes) after $retryCount retries',
          );
          break;
        }

        debugPrint(
          'FFmpeg: Waiting for overlay PNG to be written... (attempt ${retryCount + 1}/$maxRetries)',
        );
      ...
      if (overlaySize == 0) {
        debugPrint(
          'FFmpeg: CRITICAL - Overlay PNG is still empty after ${maxRetries * retryDelayMs}ms. Aborting.',
        );
        return null;
      }
      ...
      final String overlayFilter =
          '[1:v]scale=$targetWidth:$targetHeight[overlay]';
      ...
      if (hasAudio) {
        ...
        command.addAll([
          ...
          '-ar', '44100',
          '-af',
          'aresample=async=1', // Safe to use because we know audio exists
        ]);
      } else {
      ...
      command.addAll([
        ...
        // WEB OPTIMIZATION
        '-movflags',
        '+faststart', // Allows playback to start before download completes
      ...
      if (ReturnCode.isSuccess(returnCode)) {
        debugPrint(
          'FFmpeg: SUCCESS - Video with overlays saved to $outputPath',
        );
        ...
        if (encoderIdx != -1 && encoderIdx + 1 < fallbackCommand.length) {
          ...
          fallbackCommand.insertAll(encoderIdx + 2, [
            '-preset',
            'ultrafast',
            '-crf',
            '23',
        ...
        final fallbackSession = await FFmpegKit.executeWithArgumentsAsync(
          fallbackCommand,
        );
        final fallbackReturnCode = await fallbackSession.getReturnCode();
        ...
        if (ReturnCode.isSuccess(fallbackReturnCode)) {
          debugPrint(
            'FFmpeg: SUCCESS (Software Fallback) - video saved to $outputPath',
          );
          return File(outputPath);
        } else {
          final fallbackLogs = await fallbackSession.getAllLogsAsString();
          debugPrint(
            'FFmpeg: CRITICAL - Software fallback also failed: $fallbackLogs',
          );
          return null;
        }
      }
  ...
  static void processAndUpload({
    ...
    try {
      ...
      debugPrint(
        'FFmpeg (Async): Overlay PNG validated ($overlaySize bytes)',
      );
      ...
      FFmpegKit.executeWithArgumentsAsync(command, (session) async {
        ...
        if (ReturnCode.isSuccess(returnCode)) {
          ...
          // Cleanup overlay
          try {
            if (await overlayImage.exists()) await overlayImage.delete();
          } catch (_) {}
        } else {
          // HARDWARE ENCODER FALLBACK
          debugPrint(
            'FFmpeg (Async): Hardware encoder failed, retrying with libx264...',
          );
          ...
          FFmpegKit.executeWithArgumentsAsync(fallbackCommand, (
            fallbackSession,
          ) async {
            final fallbackReturnCode = await fallbackSession.getReturnCode();
            ...
            if (ReturnCode.isSuccess(fallbackReturnCode)) {
              ...
              final fallbackLogs = await fallbackSession.getAllLogsAsString();
              debugPrint(
                'FFmpeg (Async): CRITICAL - Software fallback also failed: $fallbackLogs',
              );
  ...
  static Future<Uint8List> encodeImageInIsolate(
    Uint8List rawBytes,
    int width,
    int height,
  ) async {
