...
class VibeUploadTask {
  final String id;
  final List<String> receiverIds; // 🌟 CHANGED: Support multiple receivers (Fan-Out)
...
class VibeUploadNotifier extends StateNotifier<List<VibeUploadTask>> {
      ...
      if (task.isVideo && task.videoPath != null && task.overlayPath != null) {
          ...
          VideoProcessingService.processAndUpload(
            ...
            onProcessed: (processedFile) async {
              ...
              completer.complete(task.copyWith(
  ...
  Future<void> _startUpload(VibeUploadTask task) async {
    ...
    if (task.isVideo && task.videoPath != null && task.overlayPath != null) {
      ...
      if (!await File(rawVideoPath).exists() || !await overlayFile.exists()) {
        debugPrint('❌ [VibeUpload] Video or overlay file missing for task ${task.id}');
      ...
      VideoProcessingService.processAndUpload(
        ...
        targetSize: targetSize,
        onProcessed: (processedFile) async {
          debugPrint('🎬 [VibeUpload] Video processing complete for task ${task.id}');
        ...
        onError: (err) {
          debugPrint('❌ [VibeUpload] Video processing error for task ${task.id}: $err');
          _activeTasks.remove(task.id);
  ...
  Future<void> _performFinalSend(VibeUploadTask task) async {
    ...
    if (task.audioPath != null) {
      ...
      debugPrint('🔍 [VibeUpload] Audio validation: exists=$exists, size=$size bytes, path=${task.audioPath}');
      if (!exists || size == 0) {
        _activeTasks.remove(task.id);
    ...
    if (task.imagePath != null) {
       ...
       final exists = await file.exists();
       final size = exists ? await file.length() : 0;
       debugPrint('🔍 [VibeUpload] Image validation: exists=$exists, size=$size bytes, path=${task.imagePath}');
    }

    try {
      debugPrint('🚀 [VibeUpload] Handoff to VibeService for task ${task.id} (audioOnly=${task.isAudioOnly})');
