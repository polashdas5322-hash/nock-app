...
class CloudinaryService {
  ...
  static const String uploadPreset =
      "nock_uploads"; // ⚠️ CREATE THIS IN CLOUDINARY CONSOLE!
  ...
  Future<Map<String, String?>> uploadMultiple({
    ...
    if (imageFile != null) {
      futures.add(uploadImage(imageFile).then((url) => results['image'] = url));
    }

    if (audioFile != null) {
      futures.add(uploadAudio(audioFile).then((url) => results['audio'] = url));
    }

    if (videoFile != null) {
      futures.add(uploadVideo(videoFile).then((url) => results['video'] = url));
    }
